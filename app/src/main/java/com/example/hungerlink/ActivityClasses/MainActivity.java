...
public class MainActivity extends AppCompatActivity {
    ...
    // Card views for different actions
    CardView cardDonate, cardReceive, cardProfile, cardDonationList, cardContact, cardHistory, cardLogout;

    @Override
    protected void onCreate(Bundle savedInstanceState) {
        super.onCreate(savedInstanceState);
        setContentView(R.layout.fragment_main); // Set the content view to fragment_main layout
        ...
        // Set click listeners for each card view to navigate to corresponding activities
        cardDonate.setOnClickListener(v -> {
            Intent intent = new Intent(MainActivity.this, DonateActivity.class);
            startActivity(intent);
        });
        ...
        cardLogout.setOnClickListener(v -> {
            // Handle log out logic or redirect to login screen
            Intent intent = new Intent(MainActivity.this, LoginActivity.class);
            startActivity(intent);
            finish(); // Close the main activity after logging out
        });
    ...
    public void onRequestPermissionsResult(int requestCode, @NonNull String[] permissions, @NonNull int[] grantResults) {
        super.onRequestPermissionsResult(requestCode, permissions, grantResults);
        if (requestCode == 101) { // Check if it's our permission request
            if (grantResults.length > 0 && grantResults[0] == PackageManager.PERMISSION_GRANTED) {
                // Permission was granted
                Toast.makeText(this, "Notification permission granted", Toast.LENGTH_SHORT).show();
            } else {
